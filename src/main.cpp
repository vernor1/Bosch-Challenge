...
//  std::cout << "       getXY: x " << x << ", y " << y << std::endl;
...
/*
            ...
            auto d = 6.;
            auto dt = 0.02;
//            std::cout << "Speed " << car_speed << "mph. xysd[0] " << car_x << "," << car_y << "," << car_s << "," << car_d << ".";
            for (std::size_t i = 1; i < 51; ++i) {
              auto s = car_s + i * v * dt;
              auto cartesian = GetCartesian({s, d}, map_waypoints_s, map_waypoints_x, map_waypoints_y);
//              auto xy = getXY(s, d, map_waypoints_s, map_waypoints_x, map_waypoints_y);
//              assert(xy.size() == 2);
//              std::cout << " xysd[" << i << "] " << xy[0] << "," << xy[1] << "," << s << "," << d << ".";
              next_x_vals.push_back(cartesian.x);
...
/*
            ...
            for(int i = 0; i < 50; i++) {
              auto x = car_x+(dist_inc*i)*cos(deg2rad(car_yaw));
              auto y = car_y+(dist_inc*i)*sin(deg2rad(car_yaw));
              std::cout << " xy[" << i + 1 << "] " << x << "," << y << ".";
