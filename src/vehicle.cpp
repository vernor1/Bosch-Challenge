...
const std::vector<double> Vehicle::SIGMA_S{10.0, 4.0, 2.0};
const std::vector<double> Vehicle::SIGMA_D{1.0, 1.0, 1.0};
...
Vehicle::Vehicle(const State& begin_s, const State& begin_d)
  ...
  assert(begin_s_.size() == STATE_ORDER);
  assert(begin_d_.size() == STATE_ORDER);
}
